...
for {set tn 1} {$tn<=$N} {incr tn} {
  ...
  set sql "drop table IF EXISTS t${tn}"
  puts "statement ok\n$sql\n"
  set sql "CREATE TABLE t${tn}(\n"
  append sql "  a$tn INTEGER,\n"
  append sql "  b$tn INTEGER,\n"
  append sql "  x$tn VARCHAR(40)\n"
  append sql ") DUPLICATE KEY(a$tn) DISTRIBUTED BY HASH(a$tn) BUCKETS 10 PROPERTIES ('replication_num' = '1')"
  puts "statement ok\n$sql\n"
