...
static void printResult(char * const *azResult, int nResult) {
  int nRow = nResult / nColumn;
  int i, j;
  for (i = 0; i < nRow; i++) {
    for (j = 0; j < nColumn; j++) {
      fprintf(stderr, "%s%s", azResult[i * nColumn + j], (j + 1 == nColumn ? "\n" : ", "));
    }
  }
}
...
int main(int argc, char **argv){
  ...
  int nWrong = 0;                      /* Number of wrong result */
    ...
    ** line number of the first record in sScript.startLine */
    ...
    if( strcmp(sScript.azToken[0],"statement")==0 ){
      ...
      */
      ...
      nCmd++;
      if( rc ){
        // when there is a failed, nResult 1 means we need to print error msg
        if (nResult == 1) {
          fprintf(stderr, "%s  %d query failed:\t%s\n",
          zScriptFile, sScript.startLine, zScript);
        }
        // reset nResult to avoid core dump when xFreeResults
        nResult = 0;
        pEngine->xFreeResults(pConn, azResult, nResult);
        nErr++;

        /* Don't print the result if the query failed.
        */
        if (!verifyMode) {
          sScript.copyFlag = 0;
      ...
      if( verifyMode ){
        ...
        */
        if( hashThreshold==0 || nResult<=hashThreshold ){
          char* zExpects[hashThreshold];
          for(i=0; i<nResult && sScript.zLine[0]; nextLine(&sScript), i++){
            zExpects[i] = sScript.zLine;
            if( strcmp(sScript.zLine, azResult[i])!=0){
              fprintf(stderr,"%s:%d: wrong result [%s] [%d]: %s vs %s\n", zScriptFile,
                      sScript.nLine, zScript, i, sScript.zLine, azResult[i]);
              nWrong++;
              break;
            }
          }
          if (i<nResult || sScript.zLine[0]) {
            ...
            fprintf(stderr,"%s:%d: wrong result number [%s]\n", zScriptFile,sScript.nLine, zScript);
            fprintf(stderr, "-------- Expect: -------- \n");
            printResult(zExpects, i);
            fprintf(stderr, "-------- Real: -------- \n");
            printResult(azResult, nResult);
          }
        }else{
          if( strcmp(sScript.zLine, zHash)!=0){
            fprintf(stderr, "%s:%d: wrong result hash [%s] %s vs %s\n",
                    zScriptFile, sScript.nLine, zScript, sScript.zLine, zHash);
            nWrong++;
            printResult(azResult, nResult);
            // commented by zhuming, continue to process other cases.
            // break;
          }
        }
      }else{
  ...
  */
  if(verifyMode || nErr || nSkipped || nWrong){
    fprintf(stderr, "%d result wrong %d errors out of %d tests in %s - %d skipped.",
            nWrong, nErr, nCmd, zScriptFile, nSkipped);
