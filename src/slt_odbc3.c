...
*/
static int ODBC3_perror(char *fn,
                         SQLHANDLE handle,
                         SQLSMALLINT type)
{
  SQLSMALLINT i = 0;
  ...
  do
  {
    ...
    {
      if (strstr((const char*)text, "Lost connection")) {
        fprintf(stderr,
...
static int ODBC3Connect(
  ...
  if( !SQL_SUCCEEDED(ret) ){
    ODBC3_perror("SQLAllocHandle 191", pODBC3conn->env, SQL_HANDLE_ENV);
    rc = 1;
  }
  ...
  if( !rc ){
    ...
    if( !SQL_SUCCEEDED(ret) ){
      ODBC3_perror("SQLAllocHandle 208", pODBC3conn->env, SQL_HANDLE_ENV);
      rc = 1;
    }
  }
...
static int ODBC3Query(
  ...
  int rc = 0;
  SQLRETURN ret;              /* ODBC API return status */
  // FixMe(kks): This is very strange, the query result get by xConnect is wrong
  // ODBC3_Handles* pODBC3conn = NULL;
  ODBC3_resAccum res;         /* query result accumulator */
  char zBuffer[512];          /* Buffer to render numbers */
  SQLSMALLINT columns;        /* number of columns in result-set */
  int need_print_error_msg = 0;
  ...
  ret = SQLDriverConnect(dbc, NULL, ((ODBC3_Handles*)pConn)->zConnStr, SQL_NTS,
  ...
  if( !SQL_SUCCEEDED(ret) && (ret != SQL_SUCCESS_WITH_INFO) ){
    need_print_error_msg = ODBC3_perror("SQLAllocHandle", dbc, SQL_HANDLE_DBC);
    return 1;
  }

  ret = SQLExecDirect(stmt, (SQLCHAR *)zSql, SQL_NTS);
  if( !SQL_SUCCEEDED(ret) && (ret != SQL_SUCCESS_WITH_INFO) ){
    need_print_error_msg = ODBC3_perror("SQLExecDirect", stmt, SQL_HANDLE_STMT);
    rc = 1;
  }
  ...
  if( !rc ){
    ...
    if( !SQL_SUCCEEDED(ret) && (ret != SQL_SUCCESS_WITH_INFO) ){
      need_print_error_msg = ODBC3_perror("SQLNumResultCols", stmt, SQL_HANDLE_STMT);
      rc = 1;
    }
